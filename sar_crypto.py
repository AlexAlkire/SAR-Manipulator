...
"""
Cryptography methods for encrypting/decrypting .sar files.
"""
...
"""
Context class with methods for performing encryption and decryption of .SAR files.
"""


def round_fn(s, x):
    a = s[0][(numpy.right_shift(x, 24))]
    b = s[1][(numpy.right_shift(x, 16) & 0xff)]
    c = s[2][(numpy.right_shift(x, 8) & 0xff)]
    d = s[3][(x & 0xff)]
    ret = (((a + b) ^ c) + d)
...
class Cursor:
    ...
    def next_bytes(self, count):
        ret = int.from_bytes(self.buffer[self.byte_index:self.byte_index + count], 'little')
    ...
    def next_bit(self, ):
        if self.bit_index == 0:
            ...
            self.byte_index += 1
            self.bit_index = 8
        bit = self.bit_value & 1
        self.bit_index -= 1
        self.bit_value = numpy.right_shift(self.bit_value, 1)
        return bit

    def write_bytes(self, count, _in):
        s = sys.getsizeof(_in)
        if sys.getsizeof(_in) >= count:
            for byte in _in.to_bytes(count, byteorder='little'):
                if len(self.buffer) > self.byte_index:
                    self.buffer[self.byte_index] = byte
                else:
                    self.buffer.insert(self.byte_index, byte)
                self.byte_index += 1

        elif len(_in) >= count:
            for byte in _in[0:count]:
                self.buffer[self.byte_index] = byte
                self.byte_index += 1
        else:
            raise Exception("write_bytes() error.")
...
class BlowfishContext:
    ...
    def key_schedule(self):
        ...
        for i in range(0,18,2):
            self.encrypt(lr)
            self.p[i] = int.from_bytes(lr[0:4], byteorder='little')
            self.p[i+1] = int.from_bytes((lr[4:8]), byteorder='little')

        for i in range(0,4):
            for j in range(0,256,2):
                self.encrypt(lr)
                self.s[i][j] = int.from_bytes(lr[0:4], byteorder='little')
                self.s[i][j+1] = int.from_bytes(lr[4:8], byteorder='little')

    def encrypt(self, buffer):
        for i in range(0,math.floor(len(buffer)/8)):
            left = int.from_bytes(buffer[i*8:i*8+4], byteorder='little')
            right = int.from_bytes(buffer[i*8+4:i*8+8], byteorder='little')
            for j in range(0, 16, 2):
                left ^= self.p[j]
                right ^= round_fn(self.s, left)
                right ^= self.p[j+1]
                left ^= round_fn(self.s, right)
            left ^= self.p[16]
            right ^= self.p[17]
            br = right.to_bytes(4, byteorder='little')
            bl = left.to_bytes(4, byteorder='little')
            for k in range(0,4):
                buffer[i*8+k] = br[k]
                buffer[i*8+4+k] = bl[k]
    ...
    def prs_decompress(self, buffer):
        read_cursor = Cursor(bf=buffer)
        write_cursor = Cursor()
        while True:
            flag = read_cursor.next_bit()
            if flag:
                write_cursor.write_bytes(1, read_cursor.next_bytes(1))
                continue
            flag = read_cursor.next_bit()
            if flag:
                offset = read_cursor.next_bytes(2)
                if offset == 0:
                    break
                size = offset & 7
                offset = (offset >> 3) | -0x2000
                if size == 0:
                    n = read_cursor.next_bytes(1)
                    size = n + 10
                else:
                    size += 2
            else:
                flag = 1 if read_cursor.next_bit() else 0
                size = 1 if read_cursor.next_bit() else 0
                size = (size | (flag << 1)) + 2
                offset = read_cursor.next_bytes(1)| -0x100
            for i in range(0, size):
                if offset > 0:
                    print("error offset too lorge")
                    return -1
                write_cursor.byte_index += offset
                new_byte = write_cursor.next_bytes(1)
                write_cursor.byte_index -= 1
                write_cursor.byte_index -= offset
                write_cursor.write_bytes(1, new_byte)
...
def decrypt_sar(path):
    ...
    # Remove sar and the flag byte.
    payload = file_bytes[4:]
    ctx = BlowfishContext(key=bf_key)
    ...
    # If sar needs additional PRS decompressing.
    if flag == 0x84:
        xor_payload = []
        for byte in payload:
            xor_payload.append(byte ^ 0x95)
        xor_payload = ctx.prs_decompress(xor_payload)
        payload = xor_payload
    return payload
